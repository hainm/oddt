...
import numpy as np
from oddt.spatial import distance, dihedral, angle_2v

def rotate(coords, alpha, beta, gamma):
    """ input in radians!!! """
    centroid = coords.mean(axis=0)
    coords = coords - centroid #copy here
    ...
    rot_matrix = np.array([[cos_beta*cos_gamma, sin_alpha*sin_beta*cos_gamma-cos_alpha*sin_gamma, cos_alpha*sin_beta*cos_gamma+sin_alpha*sin_gamma],
                           ...
                           [-sin_beta,          sin_alpha*cos_beta,                               cos_alpha*cos_beta]], dtype=np.float16)
...
def num_rotors_pdbqt(lig):
    ...
    for bond in lig.bonds:
        b = bond.OBBond
        #if b.IsRotor() and not b.IsAmide() and not b.IsAmidine():
        if not b.IsSingle() or b.IsAmide() or b.IsAmidine() or b.IsInRing():
            continue
        if b.GetBeginAtom().GetValence() == 1 or b.GetEndAtom().GetValence() == 1:
            continue
        i += 1
    return i
...
class vina_docking(object):
    def __init__(self, rec, lig = None, box = None, box_size = 1.):
        ...
        # constants
        self.w = np.array((-0.035579, -0.005156,  0.840245, -0.035069, -0.587439, 0.04565854))# 0.05846))# no.5 use optimized value instead of 1.923
    ...
    def set_ligand(self, lig):
        ...
        self.lig = vina_ligand(self.lig_dict['coords'].copy(), [0], self, self.box_size)

    def set_coords(self, coords):
        self.lig_dict['coords'] = coords

    def score(self, coords = None):
        return (self.score_inter(coords)*self.w[:5]).sum()/(1+self.w[5]*self.rb)
#         inter = (self.score_inter(coords)*self.w[:5]).sum()
#         total = (self.score_total(coords)*self.w[:5]).sum()
#         return total/(1+self.w[5]*self.rb)
